...
import {
  Search, AlertTriangle, FileWarning, Shield, Zap, ArrowRight, Loader2, Sparkles,
} from "lucide-react";
...
export default function LandingPage() {
  ...
  // Load suggested properties based on profile
  useEffect(() => {
    if (hasProfile && profile && profile.budgetMax > 0) {
      // Suggest properties in popular areas based on budget
      setLoadingSuggestions(true);
      const popularAreas = ["Irvine, CA", "Costa Mesa, CA", "Newport Beach, CA"];
      
      Promise.all(
        popularAreas.map(async (area) => {
          try {
            const searchParams = new URLSearchParams({ address: area });
        ...
        const allProperties = results.flat().slice(0, 6); // Get top 6 suggestions
        setSuggestedProperties(allProperties);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        ...
        {/* Suggested Properties (if profile exists) */}
        {hasProfile && profile && suggestedProperties.length > 0 && (
          <div className="max-w-xl w-full mt-8">
            <div className="flex items-center gap-2 mb-4">
              <Sparkles className="w-4 h-4 text-fuchsia-300" />
              <p className="text-sm font-mono text-fuchsia-200">
                Suggested for you
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {suggestedProperties.map((prop, idx) => (
                <button
                  ...
                  disabled={loading}
                  className="group text-left p-4 rounded-xl border border-fuchsia-400/20 bg-fuchsia-500/[0.06] hover:border-fuchsia-400/40 hover:bg-fuchsia-500/[0.12] transition-all duration-200"
                >
                  ...
                  <p className="text-sm text-foreground font-medium truncate">{prop.address}</p>
                  {prop.listPrice && (
                    <p className="text-xs text-emerald-300 mt-1 font-mono">
                      {formatCurrency(prop.listPrice)}
                    </p>
                  )}
                  {(prop.beds || prop.baths) && (
                    <p className="text-xs text-muted-foreground mt-1">
                      {prop.beds}BR / {prop.baths}BA
                    </p>
                  )}
                </button>
              ))}
